...
name_to_config = {
    cfg["name"]: cfg for cfg in [
        ...
        dict(
            ...
            n_head=32,
            n_embd=4096,
            n_query_groups=8,           # GQA: 8 KV heads, 32 query heads
            mlp_hidden_size=14336,      # not 4096*4; set explicitly to skip mlp_ratio
            norm_class_name="RMSNorm",
            mlp_class_name="LLaMAMLP",
            activation="silu",
            rotary_percentage=1.0,
        ...
        dict(
            ...
            n_head=32,
            n_embd=2560,
            mlp_hidden_size=10240,      # 4x expansion
            norm_class_name="LayerNorm",
            mlp_class_name="GptMLP",
            activation="gelu",
            gelu_approx="tanh",
            rotary_percentage=0.4,      # partial RoPE: 32 of 80 head dims
            parallel_residual=False,
            norm_eps=1e-5,
            position_embedding="rope",
            use_rope=True,
            rope_base=10000,
            weight_tying=False,
            attention_bias=True,        # Phi-2 uses biases throughout
            bias=True,
        ),
        ...
        dict(
            ...
            n_head=8,
            n_embd=2048,
            n_query_groups=1,           # MQA: 1 KV head
            mlp_hidden_size=16384,
            norm_class_name="RMSNorm",
            mlp_class_name="GemmaMLP",
            activation="gelu",
            gelu_approx="tanh",
            scale_embeddings=True,      # Gemma scales input embeddings by sqrt(n_embd)
            rotary_percentage=1.0,
            parallel_residual=False,
            norm_eps=1e-6,
            position_embedding="rope",
        ...
        dict(
            ...
            n_head=16,
            n_embd=3072,
            head_size=256,              # Gemma 7B uses head_dim=256, not n_embd//n_head=192
